...
namespace TestNau.DAL
{
    public class ItemDetailRepository
    {
        ...
        {
            string sql = $@"INSERT INTO test_na.item_detail
                            (id_cod_item, id_cod_detail, id_amount, id_price, id_total)
                            VALUES(@id_cod_item, @id_cod_detail, @id_amount, @id_price, @id_total);
                            SELECT * FROM test_na.item_detail WHERE id_code = LAST_INSERT_ID();";
            return _dbContext.getObject<ItemDetailModel>(sql, model);
        }

        public ItemDetailModel Update(ItemDetailModel model)
        {
            string sql = $@"UPDATE test_na.item_detail
                            ...
                            SELECT * FROM test_na.item_detail WHERE id_code = @id_code;";
            return _dbContext.getObject<ItemDetailModel>(sql, model);
        }
